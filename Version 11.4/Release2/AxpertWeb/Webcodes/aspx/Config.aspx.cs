...
public partial class Config : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            string SSOPathconfigfile = @" " + ScriptsPathSSO + "";
            //string directoryPathSSO = Path.GetDirectoryName(SSOPathconfigfile);
            FileInfo FileSSOfile = new FileInfo(ScriptsPathSSO);
            try
            {
                if (FileSSOfile.Exists)
                {
                    existingJsonSSO = File.ReadAllText(SSOPathconfigfile);
